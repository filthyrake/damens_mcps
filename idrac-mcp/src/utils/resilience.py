...
class CachedResponse(Generic[T]):
    ...
    Note: This class is NOT thread-safe. For concurrent access, use appropriate
    locking at the call site to prevent race conditions.
    ...
    Example usage:
        ...
        if self._system_info_cache and self._system_info_cache.is_valid():
            return self._system_info_cache.data
        
        result = self._make_request("GET", "/redfish/v1/Systems/System.Embedded.1")
        self._system_info_cache = CachedResponse(result, ttl_seconds=300)
        return result
